...
```
clean-code-skills/
├── skills/          ① Agent Skills — so an AI applies the standard (Claude Code · Codex ·
│                     Cursor · Grok · Antigravity — see `AGENTS.md` + INSTALL §1)
...
```
Per-provider paths, verification commands, and which rows were actually executed: `INSTALL.md` §1.
...
| ESLint 9.39.5 + Prettier 3.9.6 | `npm run check` in `configs/js` | configs parse · `samples/good-example.ts` = **0 problems** · the 2 deliberate bad samples = 10 problems, right rules |
